...
const remove = '春香渡邉香織松松嵜翔平奥山春花吉原健司田辺莉咲子西野入流佳玲奈徳井義実葉山奨之馬場園梓'
...
console.log('new kanji in episode 2', kanji2.filter(kanji => !kanji1.includes(kanji)).length)
...
const withCommonKanjiLines = lines.filter(line => commonKanji.some(common => line.includes(common)))
...
console.log('only kanji lines', withCommonKanjiLines.length/ linesWithKanji.length * 100)


sorted.slice(10, 20).forEach(([kanji, count]) => console.log(kanji, `https://jisho.org/search/${kanji}`, `https://hochanh.github.io/rtk/${kanji}`, count))

function getFrequentKanjis(subtitles) {
    const pairs = _.toPairs(_.countBy(subtitles.split('').filter(isKanji).filter(kanji => !remove.includes(kanji))))
    const sorted = _.sortBy(pairs, ([kanji,count]) => -count)
    return sorted
}